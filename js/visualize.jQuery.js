...
$.fn.visualize = function(options, container){
	return $(this).each(function(){
		...
		var o = $.extend({
			...
			pieRemainder: 1 // NEW: the remaining portion of the pie chart that should be colored white
		},options);
		
		//reset width, height to numbers
		o.width = parseFloat(o.width);
		o.height = parseFloat(o.height);
		if(o.pieRemainder != 1) o.colors[o.pieRemainder-1] = $(this).css('backgroundColor');
		...
		var createChart = {
			pie: function(){	
				...
				var radius = centery - o.pieMargin - 50;	
				//var centerx = canvas.width()-(radius+o.pieMargin); //Right Alignment
				var centerx = (radius+o.pieMargin); //Left Alignment
				var counter = 0.0;
				...
				$.each(memberTotals, function(i){
			        ...
			        if(percentage){
				        var labeltext = $('<span class="visualize-label">' + percentage + '%</span>')
				        	...
				        	.css(topBottom, 0);
				        if(labeltext)
				        	var label = $('<li class="visualize-label-pos' + ((dataGroups[i].color == self.css('backgroundColor')) ? ' visualize-hide-key' : '') + '"></li>')
				       			.appendTo(labels)
					...
					// If value is 0 or very close to 0, hide the label text
					if(fraction < .009){
						labeltext
							.css('display','none');
					}
		...
		if(o.appendKey){
			...
			self.find(selector).each(function(i){
				$('<li><span class="visualize-key-color" style="background: '+dataGroups[i].color+'"></span><span class="visualize-key-label' + ((dataGroups[i].color == self.css('backgroundColor')) ? ' visualize-hide-key' : '') + '">'+ $(this).text() +'</span></li>')
					.appendTo(newKey);
			});
